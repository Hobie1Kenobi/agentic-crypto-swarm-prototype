...
Full scriptable flow (set env → create wallets → deploy → fund → orchestrate → monitor → verify): see **[CELO-SEPOLIA-TESTNET.md](../celo-xrpl/CELO-SEPOLIA-TESTNET.md)** for the full 8-step flow, command reference, and troubleshooting.
...
Before launching on Celo mainnet (chain 42220), use **[PRODUCTION-READINESS.md](PRODUCTION-READINESS.md)** for: production deployment checklist, security and operational checklists, secrets handling, monitoring and alerting, wallet key management, upgrade/governance notes, economic risk, abuse/spam and rate limits, accounting/audit logs, and the recommended minimal v1 launch (no DAO; EOA or multisig owner).
