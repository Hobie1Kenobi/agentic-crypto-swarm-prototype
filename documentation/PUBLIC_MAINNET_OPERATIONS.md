...
| **Celo native x402** (`api_402`) | `eip155:42220` (Celo mainnet) or `eip155:11142220` (Sepolia dev) | CELO | `REVENUE_SERVICE_ADDRESS` (buyers pay contract via `fulfillQuery`; seller verifies tx) |
...
2. **Discovery must see HTTPS** — Set public URLs in `.env`, then refresh **`docs/endpoints.json`** so GitHub Pages and tools list the same origins as `external_commerce/discovery.py` (which overlays `X402_SELLER_PUBLIC_URL`, `T54_SELLER_PUBLIC_BASE_URL`, and `CELO_402_PUBLIC_URL`).
...
1. Run sellers locally (or on a host):
   - `npm run api:402` → `127.0.0.1:8042` (Celo native)
...
3. In **repo root `.env`** (never commit):
   ...
   - `CELO_402_PUBLIC_URL=https://your-host/celo/query`
