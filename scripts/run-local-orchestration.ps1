# Full orchestration on local Anvil — strategist → IP-generator → deployer → simulation → finance.
# Uses Anvil's pre-funded account (10,000 ETH). Deploys contracts, writes .env.local, then runs main.py.
...
try {
    ...
    $envLocalLines += "IP_GENERATOR_PRIVATE_KEY=$anvilKey"
    $envLocalLines += "DEPLOYER_PRIVATE_KEY=$anvilKey"
    $envLocalLines -join "`n" | Set-Content -Path $envLocalPath -Encoding utf8 -NoNewline:$false
    Write-Host "Wrote $envLocalPath (Anvil-funded wallets)."
...
Write-Host "Local orchestration complete. Report: simulation_report.json / simulation_report.md"
